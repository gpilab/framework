...
try:
    ...
    print(
        "Warning: Could not import gpi.config. Custom user settings from ~/.gpirc might not be applied."
    )
...
def compile_cpp_module(
    ...
    with (
        tempfile.TemporaryFile(mode="w+") as stdout_capture,
    ...
    for arg in args:
        ...
        # Case 2: Argument is a base module name (e.g., 'Grid')
            ...
            if os.path.exists(search_path):
                ...
                print(
                    f"Skipping '{arg}': Could not find '{expected_filename}' in the current directory or as an explicit _PYBIND11.cpp file."
                )
        ...
        if target_pybind_file:
            ...
            module_sources = discover_module_sources(
                target_pybind_file, base_search_dir=current_dir_for_search
            )
            ...
            print(
                f"Skipping non-_PYBIND11.cpp file: {filename_arg}. This script only builds _PYBIND11.cpp modules."
            )
    return targets
...
COMPILATION_CACHE_FILE = os.path.join(
    tempfile.gettempdir(), "gpi_make_pybind11_cache.pkl"
)
...
def save_compilation_cache(compiled_files):
    ...
    try:
        cache = {"timestamp": time.time(), "compiled_files": compiled_files}
        ...
        try:
            ...
            for include in includes:
                ...
                potential_paths.append(
                    os.path.abspath(os.path.join(current_file_dir, include))
                )
                # 2. Try common subdirectories (e.g., 'cpp/', 'include/') relative to current file's dir
                potential_paths.append(
                    os.path.abspath(os.path.join(current_file_dir, "cpp", include))
                )
                potential_paths.append(
                    os.path.abspath(os.path.join(current_file_dir, "include", include))
                )
                ...
                potential_paths.append(
                    os.path.abspath(
                        os.path.join(current_file_dir, "..", "include", include)
                    )
                )
                ...
                for s_dir in search_dirs:
                    ...
                    if os.path.exists(s_dir) and os.path.isdir(s_dir):
                        potential_paths.append(
                            os.path.abspath(os.path.join(s_dir, include))
                        )
                        potential_paths.append(
                            os.path.abspath(os.path.join(s_dir, "cpp", include))
                        )
                        potential_paths.append(
                            os.path.abspath(os.path.join(s_dir, "include", include))
                        )
                ...
                if not found_dependency_path:
                    ...
                    if glob_results:
                        found_dependency_path = os.path.abspath(
                            glob_results[0]
                        )  # Take the first match

                if (
                    found_dependency_path
                    and found_dependency_path not in processed_files
                ):
...
def get_combined_hash_for_module(pybind_file_path, base_search_dirs):
    ...
    while i < len(files_to_scan_for_cpp_and_deps):
        ...
        if current_file.endswith((".h", ".hpp")):
            ...
            if (
                os.path.exists(cpp_candidate)
                and os.path.abspath(cpp_candidate) not in all_relevant_files
            ):
    ...
    for f_path in sorted(list(all_relevant_files)):  # Sort for consistent hash
        if os.path.exists(f_path):
            try:
                ...
                print(
                    f"Warning: Could not read {f_path} for hashing, using mtime. Error: {e}"
                )
...
def discover_module_sources(pybind_file_path, base_search_dir):
    ...
    local_source_search_paths = [
        p for p in local_source_search_paths if os.path.isdir(p)
    ]
            ...
            for include_name in includes:
                ...
                for search_base in search_bases:
                    candidate_path = os.path.abspath(
                        os.path.join(search_base, include_name)
                    )
                ...
                if resolved_path:
                    ...
                    if (
                        resolved_path not in processed_files_for_includes
                        and resolved_path not in module_sources
                    ):
                        ...
                        if resolved_path.endswith(".cpp"):
                            ...
                            print(
                                f"    Discovered source file: {os.path.relpath(resolved_path, base_search_dir)}"
                            )
                            files_to_process.append(
                                resolved_path
                            )  # Also process its includes if it has any
                    ...
                    if resolved_path.endswith((".h", ".hpp")):
                        ...
                        if (
                            os.path.exists(cpp_candidate)
                            and cpp_candidate not in module_sources
                        ):
                            module_sources.add(cpp_candidate)
                            print(
                                f"    Discovered corresponding C++ source: {os.path.relpath(cpp_candidate, base_search_dir)}"
                            )
                            files_to_process.append(
                                cpp_candidate
                            )  # Add the cpp for its own dependency scan
...
def should_skip_compilation(target_info, cache):
    ...
    module_base_dir = target_info[
        "pth"
    ]  # Use the directory of the _PYBIND11.cpp as base for hash calculation
    ...
    if pybind_file in cache:
        ...
        if isinstance(cached_info, dict):
            ...
            if current_hash is not None and current_hash == cached_hash:
                print(
                    f"  Skipping {os.path.basename(pybind_file)} (unchanged with all module dependencies)"
                )
                return True
            else:
                print(
                    f"  Will compile {os.path.basename(pybind_file)} (module sources or dependencies changed)"
                )
                return False
    return False
...
def targetWalk(
    ...
    unique_search_dirs = get_search_directories(
        project_root, ignore_gpirc, ignore_sys, is_all_flag_active, working_dir
    )

    print(
        f"Searching for _PYBIND11.cpp files in {len(unique_search_dirs)} directories:"
    )
            ...
            if current_depth <= recursion_depth:
                for fil in fn_list:
                    if fil.endswith("_PYBIND11.cpp"):
                        ...
                        print(
                            f"  Discovering all sources for module '{mod_name}' (starting from {os.path.basename(full_pybind_path)})"
                        )
                        module_sources = discover_module_sources(
                            full_pybind_path, base_search_dir=path
                        )
    ...
    print(f"\nSUMMARY:")
    print(
        f"Found {len(unique_pybind_files_found_for_targets)} primary _PYBIND11.cpp files."
    )
    for t in targets:
        # Print only the base names for brevity in summary
        source_basenames = [os.path.basename(s) for s in t["all_sources"]]
        print(
            f"  Module '{t['fn']}' will be built from {len(source_basenames)} source(s): {', '.join(source_basenames)}"
        )
...
def get_search_directories(
    project_root, ignore_gpirc, ignore_sys, is_all_flag_active=False, working_dir=None
):
    """
    ...
    if is_all_flag_active:
        ...
        for d in search_dirs:
            ...
            if (
                os.path.isdir(normalized_d)
                and normalized_d not in unique_and_filtered_dirs
                and not any(
                    indicator in normalized_d
                    for indicator in system_indicators_for_filter
                )
            ):
    ...
    search_dirs.append(
        working_dir
    )  # For explicit arguments, we definitely want to search the working directory.
    ...
    if not ignore_gpirc and "Config" in sys.modules:
        try:
            if hasattr(Config, "GPI_LIBRARY_PATH") and Config.GPI_LIBRARY_PATH:
                for flib_path in Config.GPI_LIBRARY_PATH:
                    if os.path.isdir(flib_path):
                        if not any(
                            excluded in flib_path for excluded in system_indicators
                        ):
            ...
            print(
                f"Warning: Could not process Config.GPI_LIBRARY_PATH from gpi.config: {e}"
            )
    ...
    if ignore_sys:
        print(
            "Note: '--ignore-system-libs' is true. General system library locations will be ignored for linking."
        )
    else:
        print(
            "Including common system locations for library search (for dependency discovery/linking)."
        )
        pass
    ...
    for d in search_dirs:
        normalized_d = os.path.abspath(d)
        if os.path.isdir(normalized_d) and normalized_d not in unique_search_dirs:
...
class BuildConfiguration:
    ...
    def _initialize_paths(self):
        ...
        effective_gpi_prefix = (
            self._gpi_prefix
            if self._gpi_prefix is not None
            else os.environ.get("GPI_PREFIX")
        )

        if effective_gpi_prefix:
            print(f"Using GPI_PREFIX: {effective_gpi_prefix}")
            self.include_dirs.append(
                os.path.join(effective_gpi_prefix, "include", "eigen3")
            )
            ...
            if platform.system() == "Windows":
                self.include_dirs.append(
                    os.path.join(effective_gpi_prefix, "Library/include")
                )

            self.library_dirs.append(os.path.join(effective_gpi_prefix, "lib"))
            if platform.system() == "Windows":
                self.library_dirs.append(
                    os.path.join(effective_gpi_prefix, "Library/lib")
                )
        else:
            print(
                f"{Cl.WRN}Warning: GPI_PREFIX not explicitly provided or found in environment. This may affect finding core GPI libraries.{Cl.ESC}"
            )
        ...
        if "CONDA_PREFIX" in os.environ:
            ...
            print(
                f"{Cl.WRN}Warning: CONDA_PREFIX environment variable not set. Please activate your conda environment for optimal build.{Cl.ESC}"
            )
    ...
    def _load_gpirc_config(self):
        ...
        if not self.options.ignore_gpirc and "Config" in sys.modules:
            if hasattr(Config, "MAKE_LIBS"):
                self.libraries.extend(Config.MAKE_LIBS)
            if hasattr(Config, "MAKE_INC_DIRS"):
                self.include_dirs.extend(Config.MAKE_INC_DIRS)
            if hasattr(Config, "MAKE_LIB_DIRS"):
                self.library_dirs.extend(Config.MAKE_LIB_DIRS)
            if hasattr(Config, "MAKE_CFLAGS"):
                self.extra_compile_args.extend(Config.MAKE_CFLAGS)
            ...
            if hasattr(Config, "GPI_LIBRARY_PATH") and Config.GPI_LIBRARY_PATH:
                for flib_path in Config.GPI_LIBRARY_PATH:
                    if os.path.isdir(flib_path):
                        ...
                        self.include_dirs.append(
                            flib_path
                        )  # Might contain headers directly
                        self.library_dirs.append(
                            flib_path
                        )  # Might contain libs directly
                        for usrdir in findLibrariesInPath(
                            flib_path
                        ):  # Also search within python packages
    ...
    def _add_python_includes(self):
        ...
        try:
            ...
            print(
                f"{Cl.FAIL}Error: pybind11 not found. Please install it (e.g., pip install pybind11).{Cl.ESC}"
            )
        ...
        if not self.options.ignore_sys:
            ...
            if platform.system() == "Windows":
                ...
                self.libraries.extend(
                    ["fftw3_threads", "fftw3", "fftw3f_threads", "fftw3f"]
                )
    ...
    def _apply_compiler_flags(self):
        ...
        self.extra_compile_args = [
            arg for arg in self.extra_compile_args if not arg.startswith("-std=c++")
        ]
        ...
        self.extra_compile_args = [
            ...
            if not (arg == "-w" or arg.startswith("-W"))
        ]
        ...
        if not self.options.debug:
            self.extra_compile_args.extend(["-O3", "-march=native", "-DNDEBUG", "-g"])
            # Ensure GPIARRAY_ENABLE_BOUNDS_CHECKS is NOT present
            self.extra_compile_args = [
                arg
                for arg in self.extra_compile_args
                if arg != "-DGPIARRAY_ENABLE_BOUNDS_CHECKS"
            ]
        else:
            ...
            print(
                f"{Cl.OKBL}Debug mode: GPIARRAY_ENABLE_BOUNDS_CHECKS enabled, -O0 -g flags applied.{Cl.ESC}"
            )
        ...
        self.extra_compile_args = [
            arg for arg in self.extra_compile_args if arg not in openmp_flags_to_remove
        ]

        # Remove OpenMP libraries to avoid duplicates
        openmp_libs_to_remove = ["omp", "gomp", "iomp5"]
        self.libraries = [
            lib for lib in self.libraries if lib not in openmp_libs_to_remove
        ]
        ...
        if not self.options.single_threaded:
            if platform.system() == "Darwin":
                ...
                print(
                    f"{Cl.OKBL}Using OpenMP for macOS (Apple Clang: -Xpreprocessor -fopenmp).{Cl.ESC}"
                )
            ...
            print(f"{Cl.OKBL}Not using OpenMP{Cl.ESC}")
    ...
    def get_config(self):
        ...
        return {
            ...
            "extra_compile_args": remove_duplicates_preserve_order(
                self.extra_compile_args
            ),
            "runtime_library_dirs": remove_duplicates_preserve_order(
                self.runtime_library_dirs
            ),
        }
...
def do_clean(current_clean_root):
    ...
    clean_search_paths = get_search_directories(
        current_clean_root,
    ...
    for base_dir_for_clean_search in clean_search_paths:
        for path, _, fn_list in os.walk(base_dir_for_clean_search):
            for fil in fn_list:
                if fil.endswith("_PYBIND11.cpp"):
                    primary_pybind_files_for_clean.add(
                        os.path.abspath(os.path.join(path, fil))
                    )
    ...
    if os.path.exists(COMPILATION_CACHE_FILE):
        try:
            ...
            print(
                f"{Cl.FAIL}Error removing cache file {COMPILATION_CACHE_FILE}: {e}{Cl.ESC}"
            )
            return ERROR_CLEAN_FAILED
...
def do_install():
    ...
    try:
        ...
        all_potential_targets = targetWalk(
            recursion_depth=2,
            project_root=script_dir,
            ignore_gpirc=False,
            ignore_sys=False,
            is_all_flag_active=False,
        )  # Pass False here for broad search for install
        ...
        for target in all_potential_targets:
            ext_modules_to_install.append(
                Extension(name=target["fn"], sources=target["all_sources"])
            )
        ...
        setup_script_path = os.path.abspath(
            __file__
        )  # Assume make_pybind11.py IS the setup script for now
        ...
        command = [
            ...
            "--record",
            tempfile.mktemp(),  # Record installed files for potential uninstall (basic)
        ]
        ...
        env["LDFLAGS"] += " " + " ".join(
            [f"-l{l}" for l in install_settings["libraries"]]
        )
        ...
        if install_settings["runtime_library_dirs"]:
            if platform.system() == "Darwin":
                rpath_flags = " ".join(
                    [
                        f"-Wl,-rpath,{d}"
                        for d in install_settings["runtime_library_dirs"]
                    ]
                )
...
def make(GPI_PREFIX=None):
    ...
    parser.add_option(
        "-w",
        "--suppressWarnings",
        dest="suppressWarnings",  # This option is now largely superseded by default -w
        default=False,
        action="store_true",
    ...
    parser.add_option(
        ...
        default=False,
        action="store_true",
        help="disables OpenMP multi-threading",
    )
    parser.add_option(
        "--ignore-gpirc",
        dest="ignore_gpirc",
    ...
    os.environ["DISTUTILS_DEBUG"] = (
        "1" if options.distdebug else os.environ.get("DISTUTILS_DEBUG", "")
    )
    ...
    if len(args) > 0:
        ...
        if options.makeall_rdepth < 0:
            print(
                (
                    Cl.FAIL
                    + "ERROR: recursion depth is set to an invalid number."
                    + Cl.ESC
                )
            )
        ...
        search_root_for_target_discovery = CWD
        is_all_active_for_target_search = (
            True  # This flag restricts get_search_directories
        )

        print(
            f"Recursively searching for modules from: {search_root_for_target_discovery}"
        )
        targets = targetWalk(
            options.makeall_rdepth,
            search_root_for_target_discovery,
            options.ignore_gpirc,
            options.ignore_sys,
    ...
    if len(args) > 0:
        print(
            f"{Cl.WRN}Explicit targets provided. Forcing rebuild for specified modules.{Cl.ESC}"
        )
        # No filtering by cache when explicit targets are given
    elif options.force:
        print(
            f"{Cl.WRN}Force rebuild requested. Ignoring cache for all modules.{Cl.ESC}"
        )
        ...
        if len(targets) < original_target_count:
            print(
                f"{Cl.OKBL}Skipped {original_target_count - len(targets)} modules that are up-to-date.{Cl.ESC}"
            )
    ...
    print("base_compiler setting", base_compiler_settings)
    ...
    for target in targets:
        ...
        with chdir(target["pth"]):
            ...
            if target["ext"] == ".cpp":
                current_extra_compile_args = list(
                    base_compiler_settings["extra_compile_args"]
                )
                ...
                if retcode != 0:
                    ...
                    combined_module_hash = get_combined_hash_for_module(
                        target["full_filename"], [target["pth"]]
                    )
                    newly_compiled_module_info[target["full_filename"]] = {
                        "hash": get_file_hash(
                            target["full_filename"]
                        ),  # Simple hash for main file (for quick checks)
                        "dependency_hash": combined_module_hash,  # Comprehensive hash for the whole module
                        "timestamp": time.time(),
                        "module": target["fn"],
                        "all_sources_compiled": target[
                            "all_sources"
                        ],  # Store all sources that were compiled
                    }
