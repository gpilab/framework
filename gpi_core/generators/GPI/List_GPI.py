...
class ExternalNode(gpi.NodeAPI):
    ...
    def initUI(self):
        # Widgets
        self.addWidget('PushButton', 'Clear List')

        # IO Ports
        self.addInPort('in', 'PASS', obligation=gpi.REQUIRED)
        self.addOutPort('out', 'LIST')

        # persistent list object
        # NOTE: cannot be appended to from process.
        self.mylist = []
    ...
    def compute(self):
        ...
        if self.getVal('Clear List'):
            self.mylist = []
        else:
            self.mylist.append(copy.deepcopy(self.getData('in')))

        self.setData('out', self.mylist)
